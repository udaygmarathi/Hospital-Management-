...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:@localhost/psmb'
db=SQLAlchemy(app)
...
def bookings(): 
    ...
    if current_user.usertype=="Doctor":
        query=db.engine.execute(f"CALL disp_patients()")
...
def signup():
    if request.method == "POST":
        ...
        if re.fullmatch(password_pattern,password):
            ...
            flash('Please provide a valid password!!!Password must contain Capital,Small alphabets and special characters and must be atleast 5 characters long!')
            return render_template('/signup.html')
...
def details():
    ...
    posts=db.engine.execute("CALL disp_events()")
